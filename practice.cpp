...
template <typename T>
T convertPythonTo(const string& moduleName, const string& varName) {
    // Set PYTHONHOME
    ...
    if (PyLong_Check(obj)) {
        ...
        cout << "numpy array succeed loaded" << endl;
        // Accessing the NumPy array data and printing the array elements
    ...
    Py_XDECREF(numpy_array_type);
    Py_XDECREF(pandas_module);
    Py_XDECREF(pandas_dataframe_type);
    Py_Finalize();
    return v;
}

int main() {
    vector<int> result = convertPythonTo<vector<variant<long, double, string, map<string, int>>>>("test2", "arr1");
